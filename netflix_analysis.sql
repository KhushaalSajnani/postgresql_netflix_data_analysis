...
SELECT UNNEST(STRING_TO_ARRAY(listed_in,',')) as Genre, COUNT(show_id) FROM netflix GROUP BY 1 ORDER BY 2 DESC;
...
SELECT title FROM netflix WHERE COALESCE(director, 'NULL') = 'NULL'
...
SELECT UNNEST(STRING_TO_ARRAY(casts, ',')), COUNT(show_id) FROM netflix WHERE country = 'India' AND type = 'Movie' GROUP BY 1 ORDER BY 2 DESC LIMIT 10; 
